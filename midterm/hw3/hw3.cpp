...
class HI_header : public header{
    ...
    public:
        ...
        class HI_header_generator : public header_generator{
                ...
                // this constructor is only for sample to register this header type
                HI_header_generator() { /*cout << "HI_header registered" << endl;*/ register_header_type(&sample); }
            protected:
                virtual header * generate(){
...
class HI_payload : public payload {
    ...
    public:
        ...
        class HI_payload_generator : public payload_generator{
                ...
                // this constructor is only for sample to register this payload type
                HI_payload_generator() { /*cout << "HI_payload registered" << endl;*/ register_payload_type(&sample); }
            protected:
                virtual payload * generate(){ 
...
class HI_packet: public packet {
    ...
    public:
        ...
        class HI_packet_generator : public packet_generator{
                ...
                // this constructor is only for sample to register this packet type
                HI_packet_generator() { /*cout << "HI_packet registered" << endl;*/ register_packet_type(&sample); }
            protected:
                virtual packet *generate (packet *p = nullptr){
...
void GR_node::recv_handler (packet *p){
    //cout<<"test*"<<p->type()<<"*"<<endl;


    unsigned int next, DST = p->getHeader()->getDstID();;
    ...
    GR_node *n1 = dynamic_cast<GR_node*>(node::id_to_node(CUR));
    ...
    if (p->type() == "GR_packet" ){
        ...
        double min = dst(CUR, DST);
        next = CUR;
        for(n = n1->one_hop_neighbors.begin(); n != n1->one_hop_neighbors.end(); n++){
            if(n->second && dst(n->first, DST) < min){
                min = dst(n->first, DST);
                next = n->first;
            }
        }
        ...
        //cout<<"in"<<endl;
        HI_header *h1 = dynamic_cast<HI_header*> (p->getHeader());
        HI_packet *p2 = dynamic_cast<HI_packet*> (p);
        //cout<<p2->type();
        ...
        //if(dst(CUR, h1->getSrcID()) < 1.){
            
            n1->add_one_hop_neighbor(h1->getPreID());
        //}
...
int main()
{
    ...
    GR_node *n1 = dynamic_cast<GR_node*>(node::id_to_node(i));
        map<unsigned int, bool>::const_iterator n;
            
        cout<<n1->get_one_hop_neighbor_num()<<" ";
    }
    cout<<endl;*/
